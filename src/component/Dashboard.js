...
function Dashboard() {
  const { accessToken, topArtist, setTopArtist, topTrack, setTopTrack } =
    useContext(State);
    ...
    <div className="h-screen bg-zinc-900">
      ...
      <Routes>
        ...
        <Route path="/topArtist" element={<TopArtist />}>
          <Route
            index
            element={<ArtistOnTerm artist={topArtist?.long_term} />}
          />
          <Route
            path="/topArtist/long_term"
            element={<ArtistOnTerm artist={topArtist?.long_term} />}
          />
          <Route
            path="/topArtist/medium_term"
            element={<ArtistOnTerm artist={topArtist?.medium_term} />}
          />
          <Route
            path="/topArtist/short_term"
            element={<ArtistOnTerm artist={topArtist?.short_term} />}
          />
        </Route>
        ...
        <Route path="/topTrack" element={<TopTrack />}>
          ...
          <Route
            path="/topTrack/long_term"
            element={<TrackOnTerm track={topTrack?.long_term} />}
          />
          <Route
            path="/topTrack/medium_term"
            element={<TrackOnTerm track={topTrack?.medium_term} />}
          />
          <Route
            path="/topTrack/short_term"
            element={<TrackOnTerm track={topTrack?.short_term} />}
          />
        </Route>
